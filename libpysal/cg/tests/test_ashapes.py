...
class Test_Alpha_Shapes(TestCase):
    def setUp(self):
        ...
        eberly_vertices = eberly.geometry.apply(
            lambda x: np.hstack(x.xy).reshape(2, 2).T).values
        eberly_vertices = np.vstack(eberly_vertices)
        self.vertices = eberly_vertices

        self.a05 = geopandas.read_file(os.path.join(
            this_directory, 'data/alpha_05.shp')).geometry.to_numpy().item()
        self.a10 = geopandas.read_file(os.path.join(
            this_directory, 'data/alpha_tenth.shp')).geometry.to_numpy().item()
        self.a2 = geopandas.read_file(os.path.join(
            this_directory, 'data/alpha_fifth.shp')).geometry.to_numpy().item()
        self.a25 = geopandas.read_file(os.path.join(
            this_directory, 'data/alpha_fourth.shp')).geometry.to_numpy().item()
        self.a25 = geopandas.read_file(os.path.join(
            this_directory, 'data/alpha_fourth.shp')).geometry.to_numpy().item()
        circles = geopandas.read_file(os.path.join(
            this_directory, 'data/eberly_bounding_circles.shp'))
        self.circle_radii = circles.radius.iloc[0]
        self.circle_verts = np.column_stack((circles.geometry.x.values,
                                             circles.geometry.y.values))

        self.autoalpha = geopandas.read_file(os.path.join(
            this_directory, 'data/alpha_auto.shp')).geometry[0]
    ...
    def test_alpha_shapes(self):
        ...
        new_a25 = alpha_shape(self.vertices, .25).to_numpy().item()
    ...
    def test_small_n(self):
        ...
        new_triple = alpha_shape_auto(self.vertices[0].reshape(1, -1),
                                      return_circles=True)
    ...
    def test_circles(self):
        ashape, radius, centers = alpha_shape_auto(self.vertices,
                                                   return_circles=True)
