...
print(g_queen[17][1] == g_queen[1][17])



# ### attributes/function sets
...
for member in compat:
    ...
    line = [f"`{member} <generated/libpysal.graph.Graph.html#libpysal.graph.Graph.{member}>`_"]
    label = ":attr"
...
for member in w_only:
    line = [f"`{member} <generated/libpysal.weights.W.html#libpysal.weights.W.{member}>`_"]
    wa = getattr(w_queen, member)
    class_type = type(wa)
    wat = f"{class_type.__module__}.{class_type.__name__}"
