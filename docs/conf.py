...
def linkcode_resolve(domain, info):
    def find_source():
        ...
        fn = os.path.relpath(fn, start=os.path.dirname(libpysal.__file__))
    ...
    if domain != "py" or not info["module"]:
        return None
    try:
        filename = "libpysal/%s#L%d-L%d" % find_source()  # noqa: UP031
    except Exception:
        filename = info["module"].replace(".", "/") + ".py"
    tag = "main" if "dev" in release else ("v" + release)
    return f"https://github.com/pysal/libpysal/blob/{tag}/{filename}"
